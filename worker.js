...
// ==========================================
// KONFIGURASI AMAN DARI RAILWAY ENVIRONMENT
// ==========================================
...
if (!sUrl || !sKey) {
    console.error("❌ Error: SUPABASE_URL atau SUPABASE_SERVICE_ROLE_KEY belum diatur di Railway Variables!");
    process.exit(1);
}

const supabase = createClient(sUrl.trim(), sKey.trim());

const apiKeysEnv = process.env.GROQ_API_KEYS || "";
const apiKeys = apiKeysEnv.split(',').map(k => k.trim()).filter(Boolean);

if (apiKeys.length === 0) {
    console.error("❌ Error: GROQ_API_KEYS belum diatur di Railway Variables!");
    process.exit(1);
}
